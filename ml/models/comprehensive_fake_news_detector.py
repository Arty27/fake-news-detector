...
class ComprehensiveFakeNewsDetector:
    ...
    def detect(
        ...
        live_checker_result: Dict[str, Any],
    ) -> Dict[str, Any]:
        """
        ...
        factor_details["bert"] = {
            ...
            "decision": (
                "FAKE"
                if bert_result["fake_probability"] > bert_result["real_probability"]
                else "REAL"
            ),
            "confidence": max(
                bert_result["fake_probability"], bert_result["real_probability"]
            ),
        }
        ...
        factor_details["live_checker"] = {
            ...
            "top_similarity": (
                live_checker_result.get("top_matches", [{}])[0].get("similarity", 0.0)
                if live_checker_result.get("top_matches")
                else 0.0
            ),
        }
        ...
        factor_details["named_entities"] = {
            ...
            "entity_types": (
                list(set(entity["label"] for entity in ner_result))
                if ner_result
                else []
            ),
        }
        ...
        factor_details["sentiment"] = {
            ...
            "sentiment_category": self._categorize_sentiment(
                sentiment_result.get("overAll", 0.0)
            ),
        }
        ...
        return {
            ...
            "dashboard_data": self._prepare_dashboard_data(
                final_verdict, final_confidence, total_score, factor_details
            ),
    ...
    def _analyze_live_checker(self, live_result: Dict[str, Any]) -> float:
        ...
        if "strong corroboration" in decision:
            ...
            top_similarity = (
                live_result.get("top_matches", [{}])[0].get("similarity", 0.0)
                if live_result.get("top_matches")
                else 0.0
            )
    ...
    def _determine_verdict(
        self, total_score: float, factor_details: Dict[str, Any]
    ) -> tuple[str, str, str]:
    ...
    def _prepare_dashboard_data(
        ...
        return {
            ...
            "verdict_category": verdict.split()[
                0
            ].lower(),  # 'definitely', 'likely', 'suspicious'
            "factors": [
                {
                    "name": "BERT Classification",
                    "score": factor_details.get("bert", {}).get("score", 0),
                    "contribution": factor_details.get("bert", {}).get(
                        "contribution", 0
                    ),
                ...
                },
                {
                    ...
                    "contribution": factor_details.get("live_checker", {}).get(
                        "contribution", 0
                    ),
                    "decision": factor_details.get("live_checker", {}).get(
                        "decision", "N/A"
                    ),
                },
                {
                    ...
                    "contribution": factor_details.get("claim_density", {}).get(
                        "contribution", 0
                    ),
                ...
                },
                {
                    ...
                    "contribution": factor_details.get("named_entities", {}).get(
                        "contribution", 0
                    ),
                ...
                },
                {
                    ...
                    "contribution": factor_details.get("sentiment", {}).get(
                        "contribution", 0
                    ),
                    "details": factor_details.get("sentiment", {}).get(
                        "sentiment_category", "N/A"
                    ),
                },
            ],
            "summary": {
    ...
    def _apply_logical_checks(
        ...
        if "no corroboration" in live_decision and "real" in verdict.lower():
            ...
            if total_score >= 0.5:  # If score is already high enough
                return (
                    "LIKELY FAKE",
                    "HIGH",
                    f"Overridden: {reasoning} BUT no corroboration found in live news verification",
                )
            else:
                ...
                return (
                    "SUSPICIOUS",
                    "HIGH",
                    f"Overridden: {reasoning} BUT no corroboration found - requires verification",
                )
        ...
        if bert_decision == "REAL" and total_score >= 0.6:
            ...
            return (
                "SUSPICIOUS",
                "HIGH",
                f"Overridden: {reasoning} BUT high fake news indicators suggest verification needed",
            )
        ...
        if semantic_density > 0.9 and "real" in verdict.lower():
            ...
            if (
                "partial corroboration" in live_decision
                or "strong corroboration" in live_decision
            ):
                # If there's corroboration, don't override - the story might be real despite high claim density
                return (
                    verdict,
                    confidence,
                    f"{reasoning} - High claim density but corroboration evidence suggests this may be real news",
                )
            else:
                ...
                return (
                    "SUSPICIOUS",
                    "HIGH",
                    f"Overridden: {reasoning} BUT extremely high claim density suggests verification needed",
                )
        ...
        if (
            "partial corroboration" in live_decision
            or "strong corroboration" in live_decision
        ) and "fake" in verdict.lower():
            # Strong evidence suggests real news, don't override to fake
            return (
                "LIKELY REAL",
                "HIGH",
                f"Overridden: {reasoning} BUT strong corroboration evidence suggests this is real news",
            )
